...
if ($_POST['fb_sig_captcha_grade']!=1) {
	exit("Error Con las palabras de reconocimiento (CAPTCHA), intentelo de nuevo. <a href=\"" . $basepage . "/crearliga_form.php\">Volver a crear liga</a>");
}*/

if(isset($_POST['nombre'])) {
	$nombreL = strtolower(str_replace(" ","_",$_POST['nombre']));
	if (strlen($nombreL) > 30 ) {
		exit('Nombre muy largo. Maximo 30 caracteres.');
	}
	...
	exit("Nombre de liga: " . $nombreL . " ya existe! <a href=\"" . $basepage . "/crearliga_form.php\">Volver a crear liga</a>");
	}	
}
else {
	exit("Variables no seteadas... <a href=\"" . $basepage . "\">Volver a crear liga</a>");
}
?> 
